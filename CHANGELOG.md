...
- 提链与切Free 拆分为两个独立页面，侧边栏可直接进入：旧“订阅管理”页移除，`/subscription` 重定向到提链页。
- 切Free 页按页签分类展示：未切（阈值过滤 + 勾选批量切换）、已切（本机持久化记录跨重启保留，绿色“切换成功”/橙色“需关注”）、失败（切换/检查报错记录，重启后恢复的“结果不确定”切换也归入此页签等待复核）。搜索与用量阈值对三个页签全局生效，页签标题实时显示数量。
- 移除“账单信息”页面；账单信息生成并入支付链接二维码弹窗：二维码下方不再展示链接文本，改为显示账单信息（中文姓名/大写拼音/详细地址/邮政编码，逐项复制 + 一键复制全部 + 重新生成），首次打开自动生成一组，进程内跨弹窗保留。

## [1.2.48] - 2026-09-16

### 变更

- 临时浏览器工具栏「添加此账号」与「导入支付链接」合并为一个按钮：普通页面显示「添加此账号」，仅添加账号；Stripe 结账页显示「添加账号并导入链接」，先添加账号再自动导入支付链接；已添加的账号在结账页显示「导入支付链接」（只导入链接），普通页面显示「已添加」。窗口关联账号但会话识别失败时仍保留导入链接能力；链接导入失败会明确提示账号是否已添加成功。
- 临时窗口每次启动固定打开 2 个页签：`https://github.com/login` 与 `https://app.kiro.dev`（后者为活动页签，账号窗口打开即登录态）。

## [1.2.47] - 2026-09-16

### 变更

- 临时浏览器页面布局调整：「打开临时窗口」功能区移至页面顶部（错误提示之后、代理配置之前），打开窗口不再需要滚过配置表单；移除顶部的蓝色说明框（配置仅对新窗口生效等长文提示），错误/校验类提示保留。

## [1.2.46] - 2026-09-16

### 新增

- 临时浏览器「导入支付链接」：工具栏“添加此账号”旁新增按钮，在 Stripe 结账页（`https://checkout.stripe.com/c/pay/` 开头）可用，一键把地址栏完整链接（含 `#fid` 会话片段，原样保留）写入关联账号的支付链接字段。非 Stripe 页面按钮置灰且主进程独立拦截；目标账号优先取窗口关联账号（从账号列表打开），否则按窗口内登录的 Kiro 会话账号匹配，无法定位时给出明确指引。链接由主进程从活动标签页真实地址读取，超过 4096 字符拒绝导入。

### 测试

- browser-runtime 集成测试新增四条路径：按钮按 URL 前缀启用/禁用、精确写入关联账号、非 Stripe 页主进程拦截、匿名窗口报未关联账号。桌面测试 478/481（3 个失败为存量问题）。

## [1.2.45] - 2026-09-15

### 新增

- 提链出口 IP 计次：本地 SQLite 库（Node 内置 node:sqlite，零新增依赖，`pool-exit-usage.db`）记录每次提链实际使用的出口 IP——池端点 IP ≠ 真实出口（同主机相邻端口各走不同上游出口），获取端点后经两跳链向 ipify 探测实际出口并按其计次。同一出口 IP 24 小时内最多提链 2 次，超额端点自动弃用换下一个，无可用出口时明确报错并列出已用满的出口；只有真正到达 Kiro 的请求（拿到 HTTP 状态码）才计次，网络失败不占次数。
- 出口探测失败（端点过期/出口不稳）的端点自动跳过换下一个，不再中断整批获取；同一端点会话期内探测结果缓存，不重复请求。
- 设置页「测试获取一个池 IP」成功提示新增「实际出口 IP」显示。

### 测试

- 新增 pool-usage-store 测试（真 sqlite :memory:：计数、窗口边界、清理）；代理池测试扩展至 19 项（探测经端点、超领跳过、全超领报错、探测失败跳过、缓存命中）。typecheck 双侧无错误；桌面测试 478/481（3 个失败为存量问题）。
...
- 移除临时浏览器的「重复出口 IP 阈值（尝试次数）」设置及背后的重复出口检测：打开窗口不再比对上次成功窗口或运行中窗口的出口 IP、不再重试或拒绝打开；出口 IP 探测本身保留（「测试已保存配置」与窗口列表显示不受影响）。旧配置中残留的该字段在下次保存时自然丢弃。

### 测试

- 浏览器配置测试移除该字段校验；运行时测试改为「重复出口正常开窗、错误凭据仍拦截」；修复浏览器测试 preload 桩缺失 onAccountsChanged 的存量问题。typecheck 双侧无错误；浏览器 UI/运行时测试全过；桌面测试 470/473（3 个失败为存量问题）。

## [1.2.42] - 2026-09-15

### 修复

- 内置浏览器「代理」设置无法保存的问题：加载配置时草稿与已保存配置共享同一对象引用，任何表单编辑都会同步改动已保存配置，导致「有未保存修改」判定永远为 false、「保存配置」按钮始终禁用；「测试已保存配置」因主进程从未收到新配置而报「请先保存并启用自定义代理」。草稿现使用独立副本，编辑即可保存。

## [1.2.41] - 2026-09-15

### 变更

- 提链失败仅「Kiro 明确拒绝（HTTP 4xx，限流除外）」才删除账号：代理池不可达、网络错误、5xx、429 等基础设施故障保留账号并保留重试按钮，完成提示按「被拒已删除 / 失败可重试」分开统计。
- 提链并发固定为 1 并移除并发调整框：桌面面板逐账号串行，手机发起的提链 job 同样串行，与代理池逐链接取 IP 的节奏一一对应。
- 提链页移除「不选则全部」隐式语义：不勾选即无目标（提取按钮禁用），「清空」选择真正生效；卡片显示「已选 X / 可选 Y」。
- 移除「高用量跳过刷新」设置（开关与阈值）：高用量账号统一由「已废弃」分组阈值接管，相关跳过逻辑与共享判定函数一并删除。
- 自动用量刷新跳过半个间隔内刚刷新过的账号：手动批量结束后挂起的补跑轮不再重复刷用户刚刷过的账号（手动刷新不受影响；跳过会计入日志原因）。

### 测试

- 新增 isKiroRejection 判定测试；订阅管理 UI 测试新增「网络类失败保号可重试」与「HTTP 403 拒绝删号」双路径夹具并更新为显式选择语义（两阶段 PASS）；typecheck 双侧无错误；桌面测试 471/474（3 个失败为存量问题）。

## [1.2.40] - 2026-09-14

### 变更

- 提链并发固定为 1 并移除并发调整框：桌面提链面板逐账号串行执行，手机发起的提链 job 同样串行，与代理池按次取 IP 的节奏一一对应。
- 代理池改为批量提取：一次从池接口取 N 个互不相同的 IP（num 参数，默认 5、可在设置调整）放入队列逐链接消费，避免逐条提取撞上会话粘滞。实测同一池出口连续提链会被 Kiro 403，因此彻底移除 1.2.39 的「粘滞复用」——批量提取全部是最近用过的 IP 时明确报错（提示调大每次提取数量），绝不复用旧出口。
- 队列端点按池接口 time 参数（留 1 分钟余量）自动过期，过期后重新批量提取。

### 测试

- 代理池测试更新为批量语义（多行解析、一批服务多次提链、TTL 过期、重复即失败），17/17 通过；typecheck 双侧无错误；桌面测试 471/474（3 个失败为存量问题）。

## [1.2.39] - 2026-09-14

### 修复

- 代理池重试日志不带具体 IP 的问题：现在「取到最近用过的 IP」会输出端点地址，成功取得出口也会记一条含中转信息的日志。
- 池会话粘滞导致提链必失败的问题：池接口在会话有效期内（如 time=10 的 10 分钟窗口）会一直返回同一个 IP，此前重试 5 次仍重复就直接报错，等于开启池后窗口内第二次提链必失败。现在重试耗尽仍重复时复用该会话 IP（仍在有效期内）并记录警告，池轮换出新的 IP 后去重窗口照常生效。

### 测试

- 代理池测试更新为粘滞复用语义并新增「旧 IP 粘滞时挪到历史最前且不产生重复条目」用例（16/16 通过）；typecheck 无错误。

## [1.2.38] - 2026-09-14

### 修复

- 设置页「测试获取一个池 IP」显示 `undefined` 的问题：测试通道未随代理池返回结构（出口代理 + 中转代理）同步更新。
- 提链走代理池时若池出口连不上目标（池在 CONNECT 回 200 后推送明文错误应答，实测 region=Random8 的出口连不上 amazonaws、region=US 正常），此前只显示莫名其妙的 `fetch failed`；现在归因为可读错误并提示更换池接口的区域参数，订阅请求的底层错误原因也会摊平进账号错误信息。

### 变更

- 「测试获取一个池 IP」升级为全链路验证：取池 IP 后会对 Q 端点发一次轻量 https 探测，成功提示里包含出口链路状态，不再只验证「能取到 IP」。

### 测试

- 代理链回环测试新增「池在 200 后推送错误应答应归因为可读中文」用例（15/15 通过）；typecheck 双侧无错误；真网络验证 Random8 报可读错误、US 出口链路正常。

## [1.2.37] - 2026-09-14

### 新增

- 提链代理池：开启后每次生成订阅支付链接前先从池接口取一个新 IP 作为出口，并避开最近 N 个用过的 IP（窗口可配，默认 10，取到重复自动重试后仍重复则明确报错）。池接口按来源 IP 白名单鉴权，需配置一个可信 HTTP 代理访问；池端点同样按来源 IP 鉴权，实际链路为「可信代理 → 池 IP → Kiro」两跳代理。池不可用时提链直接报错，绝不静默回退直连暴露本机出口。设置 → 网络新增池配置与「测试获取一个池 IP」按钮，桌面提链面板与手机发起的提链 job 同时生效。

### 变更

- 手机端提链改回由桌面执行（桌面 job + 轮询，与切Free 同构），以共用桌面的提链代理池；移除 v1.2.31 引入的手机直连客户端与配套模型，桌面直连辅助端点保留供旧版 App 兼容。

### 测试

- 新增代理池单测（端点解析、去重与裁剪、串行获取、失败映射）与两跳代理链回环集成测试（共 14 项）；typecheck 无错误；桌面测试 468/471（3 个失败为存量问题，与本次无关）；flutter analyze 无问题，移动测试 20/21（1 个为存量失败）；真网络验证两跳出口为池出口 IP（非本机出口）。

## [1.2.36] - 2026-09-14

### 变更

- 完整 JSON 导入（含删除后重新导入）不再沿用备份里的旧「添加时间」，每次导入重新记录为本次导入时刻；导出文件仍照常携带原字段仅作参考。用量、订阅等快照字段不受影响。

### 测试

- 账号转移/备份/数据测试更新并通过（含「导入时间必须重新记录」断言，10/10）；typecheck 无错误；桌面测试 454/457（3 个失败为存量问题，与本次无关）。

## [1.2.35] - 2026-09-14

### 变更

- 提链页：已生成支付链接（待支付）的账号不再出现在待提链清单，扫描卡片新增「待支付」计数；提链失败的账号直接删除并不再出现，失败原因保留在结果列表，重试按钮随账号删除移除。
- 修复账号删除可能回滚其他账号未保存编辑的竞态：现在删除前先冲刷待写盘的本地修改（如提链刚写入的支付链接），不再被删除响应的旧快照覆盖。
- 本版随附 1.2.34 未发布的 Android 固定签名切换（1.2.34 因签名 Secrets 未配置构建中止，未产生 Release）：覆盖安装报「包名冲突」的问题自此修复，切换签名后首个版本需在手机上卸载重装一次。

### 测试

- 订阅管理 UI 测试新增提链覆盖（待支付排除、失败即删、结果保留、删除前冲刷编辑）；typecheck 双侧无错误；桌面测试 454/457（3 个失败为存量问题，与本次无关）。

## [1.2.34] - 2026-09-14

### 修复

- Android Release APK 改用固定密钥签名：发版流水线此前未配置任何签名，每次 CI 都用 runner 随机生成的 debug 密钥，各版本签名互不相同，手机端覆盖升级时报「检测到包名冲突」。现在构建前强制校验四个签名 Secrets，缺失则发版直接失败，不再静默回退随机签名。
- 本次是切换固定签名后的首个版本：手机上已装的旧版本需卸载后重装本版一次，之后 app 内检测更新即可直接覆盖升级。

### 测试

- 无应用代码变更；已用同一密钥在本地完成 release 构建并以 apksigner 核验签名证书与密钥库一致（SHA-256 864ceae9…）。

## [1.2.33] - 2026-09-13

### 新增

- 切Free 页面支持按用量百分比筛选：已用达到阈值的账号才进入待切清单，默认 15%，清空输入框则不过滤；阈值随本机记忆，账号行的用量文字补充百分比展示。

### 变更

- 切换成功的账号不再立即从清单移除：保留在列表末尾并标注「切换成功」（结果不确定标注「需关注」），复选框禁用、不参与全选与重复提交，保留至重启应用；本机防重复提交记录照旧持久化。
- 重写订阅管理 UI 测试以匹配 1.2.24 后的切Free 面板：旧测试自面板重构起引用已移除的控件标识，早已失效；新测试覆盖阈值默认/边界/清空/持久化、搜索、存储失败写入守卫与切换结果保留。

### 测试

- typecheck 双侧无错误；订阅 UI 测试双阶段（交互 + 跨进程重启）通过；桌面测试 454/457（3 个失败为存量问题，与本次无关）。
...
- 订阅门户后台会话（提链、订阅检查、切换 Free）默认直连，不再跟随系统代理；设置里的 HTTP 代理启用后仍可显式代理。
- 内置浏览器窗口（前往官网与临时浏览器）在未启用自定义代理时直连，不再进入系统代理；启用后仍走代理桥。设置页帮助文案、窗口列表与状态栏文案同步更新为「直连」。
- 设置页移除「外观」模块（深色模式、主题色、隐私打码、积分两位小数）；底层逻辑与已保存设置值继续生效，可通过「恢复默认设置」重置，README 功能清单同步更新。
...
- 修复桌面端账号标签在本地防抖保存期间被后台实时账号快照覆盖、导致刚选择的标签立即消失的问题。
- 本地待确认标签会覆盖实时快照中的旧标签字段，直至对应保存成功；保存进行中再次修改标签时保留最新选择。

### 测试

- 桌面端测试 427/427 通过，TypeScript/Vue 类型检查与生产构建通过。
## [1.2.12] - 2026-09-09
...
- 桌面端测试 427/427 通过；Flutter 分析通过、测试 9/9 通过；Android Debug APK 构建通过。
## [1.2.8] - 2026-09-05

### 界面优化

- 将列表模式中的账号状态与账号信息合并为紧凑分组，进一步缩短二者间距
- 缩短标签区与用量区的布局宽度，列表最小宽度降至 882px
- 进一步收窄展开态左侧菜单栏
- 会员等级显示移除 Kiro 前缀并统一英文单词首字母大写
- 精简关于页面，移除应用介绍、功能列表、原项目作者、赞助和交流群入口
## [1.2.7] - 2026-09-05

### 界面优化

- 进一步收窄列表模式中账号信息与用量之间的空白区域
- 将“从文件导入”和“输入 JSON 导入”合并到同一个导入账号弹窗，并支持页签切换
- 将账户管理页和设置页的“导入”下拉按钮改为普通按钮；添加账号中的快捷入口会定位到对应导入方式
## [1.2.6] - 2026-09-05

### 界面优化

- 收窄列表模式的账户标签区域和重置、到期、Token 信息区域
- 调整账户管理顶栏控件尺寸：第一行更紧凑，第二行适度增高

## [1.2.5] - 2026-09-05

### 界面优化

- 移除右侧页面顶部标题栏，将账户后台刷新进度改为右下角浮层展示
- 进一步收窄左侧导航栏，并移除“账户管理”菜单中的账户数量
- 精简批量刷新按钮及下拉菜单文案，将全选与卡片/列表切换移动到统计区域左侧
- 将账户状态和等级移到账户信息左侧并分两行显示，不再展示账号登录方式标签

## [1.2.4] - 2026-09-05

### 移除

- 移除独立“API Key 管理”菜单、页面、导入导出、测活与自动刷新，旧页面地址重定向到主页
- 移除 API Key 本地网关、端点接管/守护、调用统计以及对应的 IPC 和 preload API

### 界面优化

- 账户列表将全部操作按钮移到邮箱下方，并移除重复的账号名称行
- 扩大卡片与列表的空白区域选中范围，只有实际按钮、链接与用量区域阻止选中
- 收窄左侧导航栏，并移除账户管理页右上角的 IDE 账号检测状态与重新检测按钮

### 兼容与测试

- 启动时仅清理旧版本遗留的 IDE 端点，保留历史 Key 和统计文件，不覆盖其它工具修改过的端点
- 清理失败时保留恢复快照供下次启动重试；不改动原有账号管理、账号测活和订阅流程
- 增加真实临时配置文件的端点恢复测试，以及 IPC 注册与 preload 暴露接口的移除回归测试

## [1.2.3] - 2026-09-05

### 修复

- 恢复 API Key 管理页面、侧栏入口、网关启动恢复、状态订阅与自动刷新
- 退出 Kiro IDE 登录时只删除 Kiro 自身 token，不再清空其它 AWS SSO 缓存

### 安全

- 账号滚动备份改用 Electron 系统安全存储加密，并在启动时自动迁移旧版明文备份
- 系统安全存储不可用时停止创建包含凭证的新备份，并将已有备份权限收紧为仅当前用户可读

### 测试

- 增加 Kiro 凭证删除范围与账号备份加密格式回归测试
...
- 修复正式安装包启动后因主进程分包目录变化，无法找到 preload 和 renderer 文件而显示空白页的问题
- 修复页面加载异常时，托盘“退出程序”依赖渲染层确认框而无法退出的问题；退出确认现由主进程原生窗口处理

### 测试

- 增加运行资源路径回归测试，确保打包分包不再改变 preload 和 renderer 的定位基准
