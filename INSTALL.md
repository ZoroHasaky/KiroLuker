...
独立 API Key 管理与网关功能已移除。从启用过网关的旧版本升级后，请先启动本版本，
让应用清理仍指向旧网关的 IDE 端点，再重启 Kiro IDE。历史 Key 和统计文件不会删除。
若系统日志提示旧网关端点清理失败，恢复信息会保留；解决配置文件读取或解析问题后重新启动即可重试。
...
新版本不再提供网关启动、端口配置或强制接管入口。完全退出旧版 KiroLuker 后启动新版本，
应用会根据历史记录只还原本应用遗留的端点，再重启 Kiro IDE。其它工具后来改写的端点保持不变。
如清理失败，请查看“系统日志”，修复配置文件读取或解析问题后重试；不要删除历史恢复信息。
