...
<p align="center">
  <strong>Kiro 多账号、订阅管理与账单辅助工具</strong>
</p>

<p align="center">
  多账号集中管理 · 标签与日期筛选 · 订阅管理 · 自动刷新与流式测活 · 私密浏览器登录 · 托盘常驻
</p>

<p align="center">
  <img src="https://img.shields.io/badge/version-1.2.8-6c5ce7" alt="version">
...
入口：左侧 **订阅管理**。沿用账户管理的搜索工具栏、选择统计栏与紧凑滚动列表，结果直接显示在账号行，不再单独展示记录面板。页面只保留订阅检查和切换 Free，不再提供账号预检、选择购买计划或生成支付链接。支持搜索、当前订阅类型与续费安排组合筛选；全选只勾选当前筛选结果，已选账号被筛选隐藏后仍保持选中。全部检查与检查未切换至 Free 均不受搜索或筛选影响。类型优先采用最近门户套餐，无结果时参考本地套餐；待复核的续费安排单独归类，可一键重置两类筛选。

1. **全部检查**：顺序检查所有账号，不受账户管理页的勾选范围影响。
2. **选中检查**：仅检查本页勾选的账号，默认不勾选。
3. **检查未切换至 Free**：按最近记录跳过已确认 Free / 已安排转 Free 的账号；未检查、失败、待复核和仅取消续费的账号仍检查。
4. **切换选中至 Free**：点击后直接逐个修改所选账号中已检查且仍可切换的付费订阅，不再弹出二次确认；按钮数量为实际切换数量，自动跳过未检查、待复核或已处理的账号。可能在周期末生效，不代表退款。

- 每次检查记录最近状态、检查时间和门户快照，保存在本机，切换页面和重启后保留；失败时保留上次成功快照并标明过期参考，不假报本次成功。记录不包含账号凭证、邮箱、完整管理链接或付款资料。
- 切换前先保存“须复查”保护状态；中途退出后也不能直接重复提交。记录保存失败会明确提示并禁用切换，可通过只读检查恢复。
- 后台检查复用官网身份初始化，通过独立的内存会话调用官网 Manage plan 对应的 `GenerateSubscriptionManagementUrl`；不会占用或清空已打开的官网窗口。
- 提交后会重新读取门户状态，区分「已是 Free」「已安排周期末 Free」和「结果待复核」，不会直接把本地订阅标签改成 Free。
- 已经 Free、已安排降级或已停止续费的账号不会重复提交；结果不确定时必须先只读复查，写请求不自动重试。
- 切换可能在当前计费周期结束后生效，不代表退款；多个订阅、多席位或无法确认的门户数据会停止自动操作，请改用官网。
- 实现参考 `fanxuankai/Kiro-account-manager` 的 Stripe 门户流程（源文件中保留版本与来源）。该流程依赖门户内部协议和 Free 价格标识，Stripe 改版后可能需要适配。
- 错误标明失败阶段；明确的官网会话/凭证过期才允许刷新一次。管理接口拒绝访问不等于 Token 过期，即使官网显示 Free，也不会伪称已核实 Stripe 的续费状态。
- 自动化测试使用本地 HTTP 服务、虚拟账号及真实只读响应的脱敏协议样本，不会修改真实订阅。运行 `npm test` 验证回归，运行 `npm run build` 做类型检查及生产构建；构建后运行 `npm run test:subscription-ui`，在独立测试配置中实际加载生产页面，验证点击范围、单击执行与防重复提交、失败状态及跨进程记录保留（仅虚拟账号，不请求真实订阅）。
...
**最新版本：v1.2.8**（2026-09-05） · 变更详情见 [CHANGELOG.md](CHANGELOG.md)
...
各版本变更记录见 [CHANGELOG.md](CHANGELOG.md)，当前版本 v1.2.8，最后更新于 2026-09-05。
