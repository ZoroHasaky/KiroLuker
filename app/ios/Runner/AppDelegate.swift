...
private func checkoutFillScript(_ billing: CheckoutBillingPayload) -> String {
  ...
  (() => {
    ...
    const isCountryRelated = (text) => { const s = String(text || '').toLocaleLowerCase(); return s.includes('country') || s.includes('国家'); };
    ...
    const labelTargetInput = (terms) => Array.from(document.querySelectorAll('label')).map((label) => ({
      label, target: (label.htmlFor && document.getElementById(label.htmlFor)) || label.querySelector('input, textarea')
    ...
    const labelTargetSelect = (terms) => Array.from(document.querySelectorAll('label')).map((label) => ({
      label, target: (label.htmlFor && document.getElementById(label.htmlFor)) || label.querySelector('select')
